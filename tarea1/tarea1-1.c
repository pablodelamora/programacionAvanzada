...
//    (b+a)->propietario->nombreP = malloc(sizeof(char)*10);
    ...
    ///////////////////////////////////////////////////////////////////////////
    (b+a)->propietario->nombreP = (char*) malloc(sizeof(char)*30); //asigno memoria al arreglo de la estructura
    printf("Dame el nombre completo del propietario: ");
    scanf(" %[^\n]",(b+a)->propietario->nombreP);
    printf("Dame la edad: ");
    scanf("%d",&(b+a)->propietario->edad);
    (b+a)->propietario->rol = malloc(sizeof(char)*30);
    (b+a)->propietario->rol = "propietario";
    (b+a)->propietario->status = 1;
...
void agregarPersona(Persona *p, int a)
{
    ...
    (p+a)->nombreP = malloc(sizeof(char)*30); //asigno memoria al arreglo de la estructura
    printf("Dame el nombre completo: ");
    scanf(" %[^\n]",(p+a)->nombreP);
    printf("Dame la edad: ");
    scanf("%d",&(p+a)->edad);
    ...
    if(a>0)
    {
            ...
            if((b+idB)->ocupado < (b+idB)->nMax)
            {
                ...
                {       
                    ...
                    {
                        printf("ID - %d\t%s\t%d\t%s\t%d\n",(i+1),(p+i)->nombreP,(p+i)->edad,(p+i)->rol,(p+i)->status);
                    }//Cierre de if
                }//Cierre de for
...
{
    int j=0;
    printf("Information Barcos:\n");
    for(i=0;i<a;++i)
        printf("ID: %d Barco:%s\t%d\t%d\tLugares Disponibles: %d\n",i+1,(b+i)->nombreB, (b+i)->eslora,(b+i)->manga,((b+i)->nMax-(b+i)->ocupado));
...
void imprimir2(Barco *b, Persona *p, int a, int c)
{
    ...
    for(i=0;i<a;++i)
    {
        printf("\nID: %d Barco: %s\teslora: %d\tmanga: %d\tLugares Disponibles: %d\n",i+1,(b+i)->nombreB, (b+i)->eslora,(b+i)->manga,((b+i)->nMax-(b+i)->ocupado));
        printf("Propietario:\n%s\tedad: %d años\trol: %s\t%d\n",((b+i)->propietario)->nombreP,((b+i)->propietario)->edad,((b+i)->propietario)->rol,((b+i)->propietario)->status);

        for (j=0;j<(b+i)->ocupado;++j)
        {
            
            if((b+i)->tripulacion->status==1)
                printf("Tripulacion:\n%s\tedad: %d años\trol: %s\t%d\n",((b+i)->tripulacion+j)->nombreP,((b+i)->tripulacion+j)->edad,((b+i)->tripulacion+j)->rol,((b+i)->tripulacion+j)->status);
        }//Cierre interno
    }//Cierre de for
}//Cierre de funcion imprimir
